...
class BlueTViewController:  UIViewController, UITableViewDataSource{
    ...
    var displayalldata: [ DisplayObject ] = []
    
    private var scanner: Scanner1!
    
    func setDisplay(nom : String) -> [ DisplayObject]
    {
        for key in displayalldata
        {
            if(key.name == "MAG")
            {
                key.name = "toto"
    ...
    func getSensorTemperature(data: ([String : Sensor])) -> DisplayObject {
        ...
        for (key,value) in data
        {
            ...
            {
                if let magnetic = value as? SensorTemperature
                {
                    //
                    //display.removeAll()
                    for var cle in self.display
                    {
